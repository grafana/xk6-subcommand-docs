...
See all non-retrying assertions →
...
Soft assertions continue test execution even when they fail, marking the test as failed but allowing subsequent assertions to run. Unlike checks, soft assertions do not measure failures or emit dedicated metrics. If you need failure-related metrics, consider using checks instead:
