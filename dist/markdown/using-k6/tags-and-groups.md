...
To disable some of the preceding tags, use the `systemTags` option.
...
Thanks to some helper functions in the k6-jslib-utils project, if an executor supports the `stages` option, you can add tags with the current ongoing stage.
...
- When a taggable resource&mdash;a check, request, or custom metric&mdash;runs within a group, k6 sets the tag `group` with the current group name.
  For more info, refer to the Tags section.

Both options, the `group_duration` metric and `group tagging`, could help you analyze and visualize complex test results. Check out how they work in your [k6 result output](https://grafana.com/docs/k6/v1.5.0/misc/integrations#result-store-and-visualization).
