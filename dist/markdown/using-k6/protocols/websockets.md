...
> **Note:** [xk6-websockets](https://github.com/grafana/xk6-websockets) is an experimental module with a more standard API than `k6/ws`. It implements [the WebSockets API living standard](https://websockets.spec.whatwg.org/). While the implementation isn't complete, it uses a global event loop instead of local one.
> Currently, it's available as an experimental module `k6/experimental/websockets`. It's also likely that it will become part of the core of k6 in the future.
...
In this example, the connect() method takes a "run" function as its third parameter.
That function should accept a Socket object as its only parameter.
