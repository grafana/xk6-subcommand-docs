...
> **Note:** For all `http_req_*` metrics, **the timestamp is emitted the end of the request.**
...
The k6 browser module emits its own metrics based on the [Core Web Vitals](https://web.dev/vitals/#core-web-vitals).
...
> **Note:** Steams-related metrics (`grpc_streams*`) are available only on `k6` version `0.49.0` or higher or when using the `k6/experimental/grpc` module, which is available on `k6` version `0.45.0`.
