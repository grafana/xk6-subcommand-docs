...
| [Duration](#duration)                                        | A string specifying the total duration of the test run; together with the [vus option](#vus), it's a shortcut for a single scenario with a constant VUs executor                              |
...
| [Iterations](#iterations)                                    | A number specifying a fixed number of iterations to execute of the script; together with the [vus option](#vus), it's a shortcut for a single scenario with a shared iterations executor |
...
| [No summary](#no-summary)                                    | disables the end-of-test summary                                                                                                                                                                                                                                            |
...
| [RPS](#rps)                                                  | The maximum number of requests to make per second globally (discouraged, use arrival-rate executors instead)                                                                                                                                                |
...
| [Stages](#stages)                                            | A list of objects that specify the target number of VUs to ramp up or down; shortcut option for a single scenario with a ramping VUs executor                                                  |
...
| [Summary export](#summary-export)                            | Output the end-of-test summary report to a JSON file (discouraged, use handleSummary() instead)                                                                                       |
| [Summary mode](#summary-mode)                                | Define how detailed the end-of-test summary should be                                                                                                                                                                                                                                                                                              |
| [Summary trend stats](#summary-trend-stats)                  | Define stats for trend metrics in the end-of-test summary                                                                                                                                                                                                                   |
| [Summary time unit](#summary-time-unit)                      | Time unit to be used for _all_ time values in the end-of-test summary                                                                                                                                                                                                       |
...
By default, the server listens on `localhost:6565`. Read more on [k6 REST API](https://grafana.com/docs/k6/v1.5.0/misc/k6-rest-api). You can disable the HTTP server by setting the address value to an empty string, for example: `k6 run -a '' script.js`.
...
`http.batch()` call in a VU can make. If you have a
...
> **Note:** When running tests in k6 Cloud and using a non-default config.json file,
...
responseType to `none` for all HTTP requests. Highly recommended to be set
to `true` and then only for the requests where the response body is needed for scripting
to set responseType to `text` or `binary`. Lessens the amount of memory
...
Together with the [`vus` option](#vus), `duration` is a shortcut for a single scenario with a constant VUs executor.
...
> **Note:** This does not modify the actual HTTP `Host` header, but rather where it will be routed.
...
Pass the real system environment variables to the runtime. Available in `k6 run` and `k6 cloud run` commands.
...
Together with the [`vus` option](#vus), `iterations` is a shortcut for a single scenario with a shared iterations executor.
...
> **Caution:** This option has been deprecated since v1.3.0 and will be removed by v2.0.
> Use the `disabled` [summary mode](#summary-mode) instead.

Disables end-of-test summary generation,
including calls to `handleSummary()` and `--summary-export`.
...
learn more, have a look at the [Usage reports](https://grafana.com/docs/k6/v1.5.0/set-up/usage-collection) documentation. Available in
`k6 run` commands.
...
> **Caution:** **This option is discouraged.**
> 
> 
...
Specify source of secrets to be used within k6 script.
...
It is a shortcut option for a single scenario with a ramping VUs executor. If used together with the [VUs](#vus) option, the `vus` value is used as the `startVUs` option of the executor.
...
> **Note:** k6 v1.5.0 introduced a new opt-in format available for summary exports, designed for easier integration with external systems and analytics pipelines. You can learn more details about it in the
> `handleSummary()` function page.
...
For a better, more flexible JSON export, as well as export of the summary data to different formats (e.g. JUnit/XUnit/etc. XML, HTML, .txt) and complete summary customization, use the `handleSummary()` function.
...
See an example file on the [Results Output](https://grafana.com/docs/k6/v1.5.0/get-started/results-output#summary-export) page.

## Summary mode

> **Caution:** The `legacy` mode has been deprecated since v1.3.0, and its support will be removed by v2.0.

Define how detailed the end-of-test summary should be. Available in the `k6 run` command.
...
Define which time unit will be used for _all_ time values in the end-of-test summary. Possible values are `s` (seconds), `ms` (milliseconds) and `us` (microseconds). If no value is specified, k6 will use mixed time units, choosing the most appropriate unit for each value.
...
Add/override an environment variable with `VAR=value`in a k6 script. Available in `k6 run` and `k6 cloud` commands.
...
> **Note:** **The `-e` flag does not configure options.**
...
at the Thresholds documentation. Available in `k6 run` commands.
...
> **Caution:** Due to limitations in the underlying [go implementation](https://github.com/golang/go/issues/29349), changing the ciphers for TLS 1.3 is _not_ supported and will do nothing.
...
> **Caution:** The "Upload only" option is deprecated and will be removed in a future release. Use the `k6 cloud upload` command instead.
...
| Env             | CLI            | Code / Config file | Default                   |
| --------------- | -------------- | ------------------ | ------------------------- |
| `K6_USER_AGENT` | `--user-agent` | `userAgent`        | `Grafana k6/v1.5.0` |
...
An integer value specifying the number of VUs to run concurrently, used together with the [iterations](#iterations) or [duration](#duration) options. If you'd like more control look at the [`stages`](#stages) option or scenarios.
