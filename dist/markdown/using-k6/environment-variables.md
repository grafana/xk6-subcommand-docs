...
Rather than creating several separate scripts for these different contexts or environments, you can use [environment variables](https://grafana.com/docs/k6/v1.5.0/misc/glossary#environment-variables) to make parts of your script tweakable.
...
The recommended option to pass environment variables to your testing script is to use one or more `-e` / `--env` CLI flags
...
To override this mode, specify --include-system-env-vars.

## Configure k6 options with environment variables

You can also configure k6 options with environment variables.
...
To modify the default behavior, pass k6 options as environment variables.
...
You can confirm whether one is by checking the documentation for each option.

Note that when you define options in multiple places, there's an order of precedence that determines the option to use.
