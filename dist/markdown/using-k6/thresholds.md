...
k6 aggregates metrics according to their type.
...
> **Caution:** Do not specify multiple thresholds for the same metric by repeating the same object key.
...
If you want to set multiple thresholds for a metric, specify them with an array for the same key.

## Threshold examples to copy and paste

The quickest way to start with thresholds is to use the built-in metrics.
...
You can set thresholds per Group.
...
> **Caution:** When k6 runs in the cloud, thresholds are evaluated every 60 seconds.
