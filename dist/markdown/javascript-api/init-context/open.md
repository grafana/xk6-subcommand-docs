...
> **Caution:** `open()` can only be called from the init context. This restriction is necessary to determine the local files needed to bundle when distributing the test across multiple nodes.

> **Note:** `open()` often consumes a large amount of memory because every VU keeps a separate copy of the file in memory.
