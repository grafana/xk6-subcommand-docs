...
You must construct a `SharedArray` in the `init` context.
...
> **Caution:** Attempting to instantiate a `SharedArray` outside of the init context results in the exception `new SharedArray must be called in the init context`.
