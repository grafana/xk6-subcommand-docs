...
Registers a handler to be called whenever the specified event occurs. This method can also be used to prevent the `too many time series` error when using the k6 browser module. For more details, refer to Prevent too many time series error.
...
| Event      | Description                                                                                                                                                                                                                                                                                                     |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `console`  | Emitted every time the console API methods are called from within the page JavaScript context. The arguments passed into the handler are defined by the `ConsoleMessage` class. See the [example](#console-event-example). |
| `metric`   | Emitted every time a metric is measured and emitted for the page. The arguments passed into the handler are defined by the `MetricMessage` object. See the [example](#metric-event-example).                                |
| `request`  | Emitted every time a request made by the page. The handler gets a `Request` object that contains information about the request. See the [example](#request-and-response-events-example).                                          |
| `response` | Emitted every time a response is received by the page. The handler gets a `Response` object that contains information about the response. See the [example](#request-and-response-events-example).                               |
