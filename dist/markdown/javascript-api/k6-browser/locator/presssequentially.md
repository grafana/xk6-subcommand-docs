...
1. **Use `fill()` for simple text input**: For form fields without special keyboard handling, use fill() instead as it's significantly faster and more reliable.
...
1. **Handling special keys**: For special keys like `Enter`, `Tab`, or `Escape`, use press() instead.

### Related

- fill() - Fill text instantly without keyboard events
- press() - Press special keys or key combinations
