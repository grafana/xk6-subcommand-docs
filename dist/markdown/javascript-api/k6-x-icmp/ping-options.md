...
| preferred_ip_version | IP | Preferred IP protocol version to use for the ping request if the target is not an IPv4 or IPv6 address. Valid values are `"ip4"` and `"ip6"`. If not set, `"ip4"` protocol will be used. |
