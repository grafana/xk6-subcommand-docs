...
> **Note:** This module is implemented as an official extension and is available natively in k6, requiring no additional installation or build steps. Refer to the [extensions documentation](/docs/k6/v1.5.0/extensions/explore) for available extensions and details.
...
- The `dns.resolve()` function resolves DNS names using a specified DNS server, allowing you to test custom DNS configurations and server performance.
- The `dns.lookup()` function resolves DNS names using the system's default DNS servers, providing a way to test standard DNS resolution behavior.
...
| ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| resolve     | Resolves a DNS name to IP addresses using a specified DNS server.                                                                                                                                                    |
| lookup   | Resolves a DNS name to IP addresses using the system's default DNS servers.                                                                                                                                                                                                                                               |
