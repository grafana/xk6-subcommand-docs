...
The `csv.parse` function parses an entire CSV file at once and returns a promise that resolves to a SharedArray instance.
...
| options   | Options | An optional parameter object to customize the parsing behavior. |
...
- **Shared memory usage**: The SharedArray returned by `csv.parse` is shared among all Virtual Users (VUs), reducing memory overhead when multiple VUs access the same data.
