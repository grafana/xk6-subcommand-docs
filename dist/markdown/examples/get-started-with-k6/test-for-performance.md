...
This requires setting up a few `options` to configure the parts of the test that don't deal with test logic.
...
Scenarios schedule load according to the number of [virtual users (VUs)](https://grafana.com/docs/k6/v1.5.0/get-started/running-k6/#add-vus), number of iterations, VUs, or by iteration rate.
...
Where the smoke test defined the load in terms of iterations, this configuration uses the `ramping-vus` executor to express load through virtual users and duration.
...
> **Note:** At this point, it'd be nice to have a graphical interface to visualize metrics as they occur.
...
Finally, run a breakpoint test, where you probe the system's limits.
