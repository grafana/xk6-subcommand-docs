...
A recording will capture session data such as CSRF tokens,
...
Either you use the higher-level Response.submitForm([params]) API
or you extract necessary hidden fields etc. and build a request yourself and then send it using the
appropriate `http.*` family of APIs, like http.post(url, [body], [params]).
...
- Selection.find(selector) (the [jQuery Selector API](http://api.jquery.com/category/selectors/)
...
The jslib utils library contains an example of this kind of function,findBetween. The function uses the JavaScript built-in [String.indexOf](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/indexOf) and therefore doesn't depend on potentially expensive regular-expression operations.
...
- Response.body
- findBetween(content, left, right)
