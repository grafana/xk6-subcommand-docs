...
- Monitoring the test run to detect errors logged by k6, which could indicate limitations of the load generator machine or the [system under test](https://grafana.com/docs/k6/v1.5.0/misc/glossary#system-under-test) (SUT).
...
In some cases, this number of VUs can generate up to 300,000 HTTP [requests per second](https://grafana.com/docs/k6/v1.5.0/misc/glossary#requests-per-second) (RPS).
...
For detailed information about these settings, how to make them permanent, and instructions for macOS, check out our ["Fine tuning OS" article](https://grafana.com/docs/k6/v1.5.0/set-up/fine-tune-os).
...
To share memory between VUs, consider using SharedArray, or an external data store, such as Redis.
...
An oversight of some scripts is to test with only the [happy path](https://grafana.com/docs/k6/v1.5.0/misc/glossary#happy-path 'The default behavior that happens when the system returns no errors') in mind.
...
Refer to the ["Fine tuning OS" article](https://grafana.com/docs/k6/v1.5.0/set-up/fine-tune-os#viewing-limits-configuration).

```bash
WARN[0034] Request Failed     error="Get http://test.k6.io/: dial tcp 99.81.83.131:80: socket: too many open files"
```

> **Note:** Decide what level of errors is acceptable. At large scale, some errors are always present.
...
- k6 does not provide the functionality of a "primary" instance to coordinate the distributed execution of the test. Alternatively, you can use the [k6 REST API](https://grafana.com/docs/k6/v1.5.0/misc/k6-rest-api) and `--paused` to synchronize the multiple k6 instances' execution.
...
- [Fine tuning OS](https://grafana.com/docs/k6/v1.5.0/set-up/fine-tune-os)
