...
The following script uses the k6 HTTP module to test a single endpoint.
...
To configure the workload, use test options.
...
> **Note:** Generally, your load tests should add sleep time.
...
To ramp the request rate up or down, use the `ramping-arrival-rate` executor instead.

For all ways to model the load in k6, refer to Scenarios.
...
Checks validate conditions during the test execution.
...
In k6, you can use Thresholds to set the test pass/fail criteria.
...
To read more about data parameterization, check out the parameterization examples and Execution context variables.
...
As your testing matures, consider creating tests that combine multiple scenarios to simulate more diverse traffic.
...
To group the results of the endpoint, use URL grouping.
...
- Run the test from your private network using the k6 run command or the [Kubernetes operator](https://github.com/grafana/k6-operator). Optionally, store the test results in k6 Cloud or other external services.
- For cloud tests:
...
Even so, we recommend you get familiar with the k6 Javascript API and script your own tests.
...
If k6 doesn't support a protocol you need, you can use (or create) [extensions](https://grafana.com/docs/k6/v1.5.0/extensions).
...
- MLLP
- NATS
- and [more](https://grafana.com/docs/k6/v1.5.0/extensions/explore).
