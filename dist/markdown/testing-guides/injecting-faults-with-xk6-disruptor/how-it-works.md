...
1. The PodDisruptor selects the target pods based on the selector attributes defined in the constructor
2. The PodDisruptor attaches the xk6-disruptor-agent to each of the target pods
3. When a fault is injected (e.g. calling the injectHTTPFaults) the PodDisruptor sends a command to the agents to inject the fault in their respective pods
...
1. The ServiceDisruptor uses the definition of the service specified in the constructor to create a pod selector that matches the pods that back the service.
