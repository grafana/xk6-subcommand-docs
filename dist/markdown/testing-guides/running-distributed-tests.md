...
It has already been established that k6 can run large load tests from a single instance, but what about _multiple instances running a single test_?

Several reasons why you may wish to run a distributed test include:

- Your [system under test](https://grafana.com/docs/k6/v1.5.0/misc/glossary#system-under-test) (SUT) should be accessed from multiple IP addresses.
...
[k6-operator](https://github.com/grafana/k6-operator) is an implementation of the [operator pattern](https://grafana.com/docs/k6/v1.5.0/misc/glossary#operator-pattern) in Kubernetes, defining [custom resources](https://grafana.com/docs/k6/v1.5.0/misc/glossary#custom-resource) in Kubernetes.
...
> **Note:** This process can be performed on a local Kubernetes cluster running within [Docker](https://docs.docker.com/get-docker/)!
...
> **Note:** Besides privileged access to a Kubernetes cluster, installation will require that the system performing the installation has the following tools installed:
...
> **Note:** While creating scripts, [run them locally](https://grafana.com/docs/k6/v1.5.0/get-started/running-k6#running-local-tests) before publishing to your cluster.
...
> **Caution:** Limitations exist on how large your test script can be when deployed within a `ConfigMap`.
...
This mechanism is ideal when breaking up monolithic scripts into reusable modules.

> **Note:** Organizing your test scripts was part of the discussion during [episode #76](https://www.youtube.com/watch?v=zDtEzp_JUOE&utm=k6-guides) of k6 Office Hours.

> **Caution:** When using a `PersistentVolume`, the operator will expect all test scripts to be contained within a directory named `/test/`.
...
Specifically, the main elements defined within the `TestRun` object relate to the name and location of the test script to run, and the amount of [parallelism](https://grafana.com/docs/k6/v1.5.0/misc/glossary#parallelism) to utilize.

> **Note:** The `TestRun` custom resource provides many configuration options to control the initialization and execution of tests.
...
When the test script to be executed is contained within a `ConfigMap` resource, we specify the script details within the `configMap` block of [YAML](https://grafana.com/docs/k6/v1.5.0/misc/glossary#yaml).
...
The operator will split the workload between the pods using [execution segments](https://grafana.com/docs/k6/v1.5.0/misc/glossary#execution-segment).

> **Caution:** It is important that the `ConfigMap` and `CustomResource` are created in the same [Namespace](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/).
...
> **Caution:** It is important that the `PersistentVolumeClaim` and `CustomResource` are created in the same [Namespace](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/).
...
We can pass this data as [environment variables](https://grafana.com/docs/k6/v1.5.0/misc/glossary#environment-variables) for use with each pod executing your script.
...
> **Note:** The above YAML introduces the `runner` section. This section applies to each pod that will be running a portion of your test, based upon the desired `parallelism`.
...
k6 options can be specified in many ways, one being the command-line.
...
With the above arguments, we're adding a test-wide custom tag to metrics and changing the output format of logs to [JSON](https://grafana.com/docs/k6/v1.5.0/misc/glossary#json).

> **Note:** Be sure to visit the options reference for a listing of available options.
...
> **Note:** If you make use of real-time results output, e.g. Prometheus Remote Write, use the `cleanup` option to automatically remove resources upon test completion as with the following example:
> ```yaml
> apiVersion: k6.io/v1alpha1
> kind: TestRun
> metadata:
